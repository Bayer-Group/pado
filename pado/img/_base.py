...
class Image:
    ...
    def mpp(self):
        try:
            ...
            # sometimes mpp not directly available in metadata (properties)
            properties = self._image_backend.image_metadata()
            resolution_unit = properties['tiff.ResolutionUnit']
            resolution_x, resolution_y = float(properties['tiff.XResolution']), float(properties['tiff.YResolution'])

            assert resolution_unit == 'centimeter'

            mpp_x = 1 / ((1e-6 / 1e-2) * resolution_x)
            mpp_y = 1 / ((1e-6 / 1e-2) * resolution_y)
    ...
    def levels(self):
        try:
            ...
            # sometimes mpp not directly available in metadata (properties)
            return tuple(self.level_mpp_map)

    @property
    def level_mpp_map(self):
        try:
            return self._image_backend.level_mpp_map
        except KeyError:
            # sometimes mpp not directly available in metadata (properties)
    ...
    def get_region(self, location_xy: Tuple[int, int], region_wh: Tuple[int, int], *,
        ...
        if mpp_xy is None and level is not None:
            if level == 0:
                ...
                lvl_mpp = self.level_mpp_map[level]
