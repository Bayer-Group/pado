...
class TiffFileImageBackend(ImageBackend):
    ...
    def open(self):
        ...
        try:
            self._slide = self._stack.enter_context(TiffSlide(os.fspath(self._fspath)))
    ...
    def image_metadata(self) -> Mapping[str, Any]:
        ...
        md = {
            'width': dimensions[0],
            'height': dimensions[1],
            'objective_power': _props_get(tiffslide.PROPERTY_NAME_OBJECTIVE_POWER),
            'mpp_x': _props_get(tiffslide.PROPERTY_NAME_MPP_X),
            'mpp_y': _props_get(tiffslide.PROPERTY_NAME_MPP_Y),
            'downsamples': list(self._slide.level_downsamples),
            'vendor': _props_get(tiffslide.PROPERTY_NAME_VENDOR),
            # Optional props
            'background_color': _props_get(tiffslide.PROPERTY_NAME_BACKGROUND_COLOR),
            'quickhash1': _props_get(tiffslide.PROPERTY_NAME_QUICKHASH1),
            'slide_comment': _props_get(tiffslide.PROPERTY_NAME_COMMENT),
            'bounds_x': _props_get(tiffslide.PROPERTY_NAME_BOUNDS_X),
            'bounds_y': _props_get(tiffslide.PROPERTY_NAME_BOUNDS_Y),
            'bounds_width': _props_get(tiffslide.PROPERTY_NAME_BOUNDS_WIDTH),
            'bounds_height': _props_get(tiffslide.PROPERTY_NAME_BOUNDS_HEIGHT),
        }
    ...
    def level0_mpp(self) -> Tuple[float, float]:
        return (
            self._slide.properties[tiffslide.PROPERTY_NAME_MPP_X],
            self._slide.properties[tiffslide.PROPERTY_NAME_MPP_Y],
        )
